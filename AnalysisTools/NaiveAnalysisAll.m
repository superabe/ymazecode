function [ result, ratematrix, Bbins, Tbins ] = NaiveAnalysisAll( allneuron,baseline_period, test_period, Bbin, Tbin )
% This function is similar to NaiveAnalysis_2 with one difference:
% The input argument allneuron is return from getTimestamp,merge function
% containing the timestamp of neurons from all the rats.
neuron = divide(allneuron);   % divide all the neruons
Bbins =  ceil(baseline_period / Bbin);
Tbins =  ceil(test_period / Tbin);
...
for i = 1:length(neuron)
    ...
    for j = 1:28
        if upper_bound(j) > 0 && upper_bound(j + 1) > 0  && upper_bound(j + 2) > 0  % two consecutive bins bigger than the upper_bound.
            ...
            result(i,3) = j + Bbins;                           % return the first  bin appears hyperactivity
            break;
        end
    end
    for j = 1:28
...
%% plot the ratematrix
figure
h = bar3(ratematrix(:,1:Bbins + Tbins)');
for i = 1:length(neuron)
    line([i, i],[0,Bbins + Tbins],[BaselineCI(i,1),BaselineCI(i,1)],'color','r','linestyle','--','linewidth', 2);
    line([i, i],[0,Bbins + Tbins],[BaselineCI(i,2),BaselineCI(i,2)],'color','r','linestyle','--','linewidth', 2);
end

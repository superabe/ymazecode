function [ result, ratematrix ] = Plot3Dpp2( allneuron ,baseline_period, test_period, Bbin, Tbin)
% This function is similar to Plot3D, but specifically for pp2 recording.
% Plot the time course of all the neurons from pp2 treatment
% Neurons are divided into three types: 
% Up regulated, Remained unchange, down regulated. 
%
[result, ratematrix,Bbins, Tbins ] = NaiveAnalysispp2(allneuron,baseline_period, test_period, Bbin, Tbin);
% %% plot the rate matrix
% figure();
% h1 = bar3(ratematrix(result(result(:,2) == 1, 1),1:35)');
...
% 
% figure();
% h2 = bar3(ratematrix(result(result(:,2) == -1, 1),1:35)');
...
% 
% figure();
% h3 = bar3(ratematrix(result(result(:,2) == 0, 1),1:35)');
...
% 
% figure();
% h3 = bar3(ratematrix(result(result(:,2) == 2, 1),1:35)');
...
% Add labels
hTitle  = title ('Neuron Activity Change After MPH Injection for 3h');
hXLabel = xlabel('Time(min)');
hYLabel = ylabel('Relative Firing Rate');

% Add legend
hLegend = legend([error, error2, error1], ...
